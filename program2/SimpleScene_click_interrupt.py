...
# if savedCount turns six, start rollercoasting
savedCount = -1
initialCow = np.array([
    [0, 0, 0, 0],
    [0, 0, 0, 0],
...
def display():
    ...
    # if all six points are saved, rotate the cow for three times
        ...
        getCowDir(getTranslation(cow2wld), getTranslation(splinepos))
        # move the cow
...
def onMouseButton(window, button, state, mods):
    ...
    if button == glfw.MOUSE_BUTTON_LEFT:
        if state == GLFW_DOWN:
            ...
            isDrag = H_DRAG
            if savedCount == 6 and cursorOnCowBoundingBox:
                # interrupt rollercoasting
                savedCount = -1
                savedLoc.clear()

            elif 0 <= savedCount and savedCount <= 5 and cursorOnCowBoundingBox:
                # save point
                savedCount += 1
